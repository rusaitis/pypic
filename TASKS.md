...
- [ ] **Step 44: symplectic + tricubic field-line tracer for bounded-drift Poincaré sections**
  Two opt-in upgrades to `pypic.traces` that address the dominant error sources in long-time field-line integration. Both are additive: they sit *next to* the existing `trace_field_line_adaptive` / `trace_field_lines_adaptive` (Dormand-Prince 5(4) + trilinear interpolation), which stays the default. Diagnostic Poincaré sections (Step shipped pre-44 — see `pypic.traces._poincare`) work fine with the current stack; this step targets the fusion-grade island-width measurement use case where ~5% secular radial drift per ~100 orbits matters.
  ...
  **Depends on:** existing `pypic.traces` infrastructure (`VectorFieldInterpolator`, `FieldLine`, termination logic). No new mandatory dependencies — scipy already ships cubic `RegularGridInterpolator`.
