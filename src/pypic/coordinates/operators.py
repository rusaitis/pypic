...
def _require_positive_spacing(d1: float, d2: float, d3: float | None) -> None:
    """Raise if any given grid spacing is not positive.

    ``d3 = None`` marks a 2D field, where there is no third spacing to
    check rather than an invalid one.
    """
    if d1 <= 0 or d2 <= 0 or (d3 is not None and d3 <= 0):
