...
def to_toml(cfg: IPic3DConfig) -> str:
    ...
    — these can be patched by the consumer if total runtime is known.
    ...
    lines.append("[run]")
    run_name = cfg.simulation_name or "ipic3d_run"
    ...
    lines.append(f"lower = [{g.origin[0]}, {g.origin[1]}, {g.origin[2]}]")
    ...
    # reference_density is placeholder (1.0e18 m⁻³ typical lab PIC). The
    # iPIC3D .inp carries qom/B0/rho in code units only; physical density
    # is set by the user via simulation.toml override.
    lines.append("reference_density = 1.0e18")
    if cfg.c != 1.0:
    ...
    lines.append("[physics.pic.solver]")
    lines.append('scheme = "semi-implicit"')
    lines.append(f"implicitness = {cfg.th}")
    lines.append("")
