...
def _write_phdf5(base: Path, stored: dict[str, np.ndarray]) -> None:
    ...
    ipic_p_map = {
        "P11": "pXX", "P12": "pXY", "P13": "pXZ",
        "P22": "pYY", "P23": "pYZ", "P33": "pZZ",
    }
...
def _write_h5hut(base: Path, stored: dict[str, np.ndarray]) -> None:
    ...
    with h5py.File(h5_path, "w") as f:
        ...
        def _write_field(name: str, data: np.ndarray, *, use_float32: bool = True) -> None:
...
def main() -> None:
    ...
    cq_a = load_conserved_quantities(
        OUTPUT_DIR / "phdf5" / "ConservedQuantities.txt"
    )
    ...
    cq_b = load_conserved_quantities(
        OUTPUT_DIR / "h5hut" / "info-conserved"
    )
